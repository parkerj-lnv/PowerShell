<#
	...
	.DESCRIPTION
		Script will download the latest version of System Update or Thin Installer from Lenovo's support site, create a ConfigMgr Application/Deployment Type, and distribute to a Distribution Point
    ...
    .EXAMPLE
        .\Create-SystemUpdateApplication -SystemUpdateSourcePath "\\Share\Software\Lenovo\SystemUpdate\5.07.88" -ThinInstaller -ThinInstallerSourcePath ""\\Share\Software\Lenovo\ThinInstaller\1.3.00018"
...
param
(
    [Parameter(Mandatory = $false, HelpMessage = "Specify the UNC path where System Update will be downloaded to")]
    [ValidateNotNullOrEmpty()]
    [String]$SystemUpdateSourcePath,

    [Parameter(Mandatory = $false, HelpMessage = "Specify the UNC path where ThinInstaller will be downloaded to")]
    [ValidateNotNullOrEmpty()]
...
If ($ThinInstallerSourcePath)
    {
        If ((Get-AuthenticodeSignature -FilePath $ThinInstallerSourcePath\$tiExe).SignerCertificate.Thumbprint -ne $tiThumbprint)
            {
                Write-Error "Certificate thumbprints do not match. Exiting out" -ErrorAction Stop
            }
    }
